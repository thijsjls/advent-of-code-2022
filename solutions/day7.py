...
def init_dir(dir: Dir, data: list) -> Dir:
    for item in data:
        x = item.split()
        if x[0] == 'dir':
...
def cd(dir: Dir, cmd: str) -> Dir | None:
    if cmd == '..':
        return dir.parent
    else:
...
class FileSystem(object):
    ...
    def __init__(self, data='', root=Dir('/')):
        ...
        for cmd in data.split('$')[2:]:
            if cmd[:3] == ' ls':
                self.current_dir = init_dir(self.current_dir, remove(cmd[3:].split('\n'), ''))
                self.dir_list.append(self.current_dir)
            else:
                self.current_dir = cd(self.current_dir, re.sub('[\n ]', '', cmd[3:]))
